...
// probability distributions, with option-pricing capability
...
public:
	...
	virtual double VolVega
		(double strike, const OptionType_& type)
	const = 0;
	// support hedge computation
	virtual Vector_<String_> ParameterNames() const = 0;
	virtual std::map<String_, double> ParameterDerivatives
		(double strike, 
		 const OptionType_& type,
		 const Vector_<String_>& to_report)
	const = 0;
};
