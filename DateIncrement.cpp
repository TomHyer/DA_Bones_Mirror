...
namespace
{
#include "MG_SpecialDay_enum.h"
#include "MG_SpecialDay_enum.inc"
	...
	{
		int yy = Date::Year(from), mm = Date::Month(from), dd = Date::Day(from);
		if (forward ? dd >= 20 : dd <= 20)	// can't stay in same month
		{
			do
			{
				mm += forward ? 1 : -1;
	...
	{
		int yy = Date::Year(from), mm = Date::Month(from), dd = Date::Day(from);
		if (forward ? dd >= IMMDay(yy, mm) : dd <= IMMDay(yy, mm))	// can't stay in same month
		{
			do
			{
				mm += forward ? 1 : -1;
	...
	Date_ SpecialDay_::Step(const Date_& from, bool forward) const
	{
		...
		{
		default:
			assert(!"Invalid SpecialDay type");
		case Value_::CDS:
...
#include "MG_DateStepSize_enum.h"
#include "MG_DateStepSize_enum.inc"

	Date_ DateStepSize_::operator()(const Date_& from, bool forward, int n_steps, const Holidays_& hols) const
	{
		...
		{
		default:
			assert(!"Invalid DateStepSize");
		case Value_::Y:
			retval = Date::AddMonths(retval, 12 * n_steps * sign);
			break;
		case Value_::M:
			retval = Date::AddMonths(retval, n_steps * sign);
			break;
		case Value_::CD:
			retval = retval.AddDays(n_steps * sign);
			break;
		case Value_::BD:
			...
			{
				retval = forward 
						? Holidays::NextBus(hols, retval.AddDays(1)) 
						: Holidays::PrevBus(hols, retval.AddDays(-1));
			}
		}
		return forward ? Holidays::NextBus(hols, retval) : Holidays::PrevBus(hols, retval);
	}
	...
	struct IncrementCompound_ : Composite_<const Date::Increment_>
	{
		template<class F_> Date_ Apply(Date_ date, const F_& func) const
		{
			for (const auto& inc : contents_)
				date = (inc.get()->*func)(date);
			return date;
		}
		Date_ FwdFrom(const Date_& date) const { return Apply(date, &Increment_::FwdFrom); }
		Date_ BackFrom(const Date_& date) const { return Apply(date, &Increment_::BackFrom); }
	};
}	// leave local
...
Handle_<Date::Increment_> Date::NBusDays(int n, const Holidays_& hols)
{
   ...
   return make_handle<IncrementMultistep_>(n, BD, hols);
}
...
Handle_<Date::Increment_> Date::ParseIncrement(const String_& src)
{
	...
	if (src.find('&') != String_::npos)
	{
		...
		std::unique_ptr<IncrementCompound_> retval(new IncrementCompound_);
		for (const auto& ss : subs)
			retval->Append(ParseIncrement(ss));
		return retval.release();
	}
	// OK, it's a single increment
	REQUIRE(!src.empty(), "Increment is empty");
	auto nonNumeric = src.find_first_not_of("0123456789");
	if (nonNumeric == 0)	// non-numeric start means a special day, like "IMM"
		return new IncrementNextSpecial_(SpecialDay_(src));

	auto sc = src.find(';');	// check for ';' holidays syntax
	Holidays_ hols(Holidays::None());
	if (sc != String_::npos)
		hols = Holidays_(src.substr(sc + 1));
	const int n = String::ToInt(src.substr(0, nonNumeric));
	DateStepSize_ step(src.substr(nonNumeric, sc - nonNumeric));
	return new IncrementMultistep_(n, step, hols);
}
