...
namespace
{
	static std::deque<String_>& TheTempMemory()
	{
      RETURN_STATIC(std::deque<String_>);
	}
...
LPSTR GetMemoryForExcel(size_t bytes)
{
...
Date_ Excel::ToDate(const OPER_* src)
{
	return ToDate(src, false).get();
}
...
namespace
{
   template<class T_> auto ReadVector(const OPER_* src, bool optional, T_ read_scalar)->Vector_<decltype(read_scalar(src))>
   {
      ...
      // ok, it's a multi
      REQUIRE(src->val.array.rows == 1 || src->val.array.columns == 1, "Invalid input size -- one-dimensional vector expected");
...
namespace
{
	...
	template<class E_> Matrix_<E_> ToMatrix(const OPER_* src, bool optional, bool greedy, E_(*construct_element)(const OPER_*))
	{
		Matrix_<E_> retval;
		switch (src->xltype)
		{
      ...
      case xltypeNil:
		...
		// ok, it's a multi
		auto size = greedy 
				? pair<int, int>(src->val.array.rows, src->val.array.columns)
				: NonblankSize(src);
	...
	String_ StringMatrixEntry(const OPER_* src)
	{
		return Excel::ToString(src, true);
	}
	Cell_ CellMatrixEntry(const OPER_* src)
	{
		return Excel::ToCell(src, true);
	}
		...
		const char* arg_name)
	{
		...
		{
			display = display.substr(KNOCK_ON.size() - 1);	// throw away the restatement of the fact that it's an error
...
{
	return ToMatrix<String_>(src, optional, false, StringMatrixEntry);	// strings within a matrix are thus allowed to be empty
}
...
template<typename T_> auto ReaderExtract(const OPER_* src, int i_row, int i_col, T_ transform) -> decltype(transform(src))
{
...
template<typename T_, typename D_> D_ ReaderExtractOrDefault(const OPER_* src, int i_row, int i_col, T_ transform, D_ def_val)
{
   assert(src->xltype & xltypeMulti && i_row < src->val.array.rows);
...
{
   return ReaderExtractOrDefault(src_, iRow_, i_col, [](const OPER_* src){return Excel::ToDouble(src); }, def_val);
}
...
{
   return ReaderExtractOrDefault(src_, iRow_, i_col, [](const OPER_* src){return Excel::ToInt(src); }, def_val);
}
...
void Excel::Retval_::LoadBase(_ENV, const Vector_<Handle_<Storable_> >& v)
{
	...
	Transform(v, [&](const Handle_<Storable_>& h)->String_{ return repo->Add(h, ERASE); }, &toStore);
	Load(toStore);
}
...
OPER_* Excel::Retval_::ToXloper(const String_& format) const
{
	if (format.empty())
		return ToXloper();
	Retval_ helper;
	helper.Load(Matrix::Format(Apply(std::ptr_fun(AddressOf<Matrix_<Cell_>>), values_), format));
...
struct XLFunc_
{
	...
	bool volatile_;
	XLFunc_(const String_& dll_name, const String_& c_name, const String_& xl_name, const String_& help, const String_& arg_types, const String_& arg_names, const Vector_<String_>& arg_help, bool is_volatile) : dllName_(dll_name), cName_(c_name), xlName_(xl_name), help_(help), argTypes_(arg_types), argNames_(arg_names), argHelp_(arg_help), volatile_(is_volatile) {}
};

static Vector_<XLFunc_>& TheFunctions()
{
   RETURN_STATIC(Vector_<XLFunc_>);
}
...
struct XLMenuItem_
{
	...
	String_ subhead_;
	String_ text_;
	XLMenuItem_(const String_& head, const String_& subhead, const String_& text) : head_(head), subhead_(subhead), text_(text) {}
};
...
#include "MG_Format_public.inc"
#include "MG_PasteWithArgs_public.inc"
