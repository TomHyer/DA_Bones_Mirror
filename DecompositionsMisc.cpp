...
namespace
{
	...
	{
		Vector_<> vals_;
		Diagonal_(const Vector_<>& vals) : vals_(vals) {}

		int Size() const override { return vals_.size(); }

		void XMultiply_af(const Vector_<>& x, Vector_<>* b) const override { Transform(x, vals_, std::multiplies<double>(), b); }
		void XSolve_af(const Vector_<>& b, Vector_<>* x) const override { NOTE("Dividing by diagonal matrix");  Transform(b, vals_, ProtectedDivide_(), x); }
		Vector_<>::const_iterator MakeCorrelated(Vector_<>::const_iterator iid_begin, Vector_<>* devs) const override
		{
			const int n = Size();
			devs->Resize(n);
	...
	{
		SquareMatrix_<> vals_;
		LowerTriangular_(const SquareMatrix_<>& vals) : vals_(vals) {}

		int Size() const override { return vals_.Rows(); }
