...
public:
   ...
   String_ ExtractString(int i_col, const String_& def_val) const { return Exists(i_col) ? ExtractString(i_col) : def_val; }
   ...
   template<class T_> T_ ExtractEnum(int i_col, const T_& defval) const { return Exists(i_col) ? ExtractEnum<T_>(i_col) : defval; }
