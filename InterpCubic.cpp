...
#include "MG_Cubic1_Write.inc"
	struct Cubic1_ : Interp1_
	{
		Vector_<> x_, f_, fpp_;
		double operator()(double x) const override;
		bool IsInBounds(double x) const override { return x >= x_.front() && x <= x_.back(); }	// simply forbid extrapolation
		...
		{
         Cubic1::XWrite(dst, name_, x_, f_, fpp_);
		}
	};
	...
	double Cubic1_::operator()(double x) const
	{
		...
		return a * f_[iLT] + b * f_[iGE] - a * b * ((1.0 + a) * fpp_[iLT] + (1.0 + b) * fpp_[iGE]) * Square(h) / 6.0;
	}
	...
	fpp_(f_.size())
	{
		assert(x_.size() > 2 && IsMonotonic(x_));
		assert(x_.size() == f_.size());
		const int n = x_.size();
		Vector_<> u(n - 1);
		switch (lhs.order_)   // set left boundary
		{
		default:
			assert(!"Invalid boundary order");
		case 1:
		   {
